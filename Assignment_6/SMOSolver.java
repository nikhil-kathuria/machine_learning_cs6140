...
public class SMOSolver {
    ...
    // Set for Storing rowIDs, where Alpha is not bounded i.e. between 0 and C
    HashSet<Integer> nonBounded;


    // Spam tol = 0.01, C = 1 , epsilon = 0.001 -> 92.6 %
    // Digits tol = 0.01, C = .5, epsilon = 0.001  - 10 % percent data  - 88.5
    // Digits tol = 0.01, C = .5, epsilon = 0.001  - 10 % percent data  - 88.6
    // Digits tol = 0.01, C = .5, epsilon = 0.001  - 1 % percent data  - 84.5

    double tol = 0.1 ;   // Tolerance limit for KKT conditions. Recognition Systems do not require high
    double C = 1;
    ...
    public double computeH(double alpha1, double alpha2, double l1, double l2 ){
        if (l1 != l2){
            return Math.min(C, C + alpha2 - alpha1);
        } else {
            return Math.min(C, alpha2 + alpha1);
        }
    }
    ...
    public void updateWeights(double alpha1, double alpha2, double a1, double a2, int i1, int i2){
        // Check update condition
        double x1 = (a1 - alpha1) * labels[i1];
        double x2 = (a2 - alpha2) * labels[i2];
    ...
    public void updateThreshold(double alpha1, double alpha2, double a1, double a2, int i1, int i2){
        double b1;
        double b2;

        double diff1 = a1 - alpha1;
        double diff2 = a2 - alpha2;

        b1 = B + cached[i1] +  labels[i1] * diff1 * KC.k11 + labels[i2] * diff2 * KC.k12;

        b2 = B + cached[i2] + labels[i1] * diff1 * KC.k12 + labels[i2] * diff2 * KC.k22;
    ...
    public void updateAlphas(double alpha1, double alpha2, int i1, int i2){
        ...
        if (alpha2 > 0 && alpha2 < C){ nonBounded.add(i2) ;}
        else {nonBounded.remove(i2);}
    ...
    public void updateALL(double alpha1, double alpha2, double a1, double a2, int i1, int i2){
        updateThreshold(alpha1, alpha2, a1, a2, i1, i2);
        updateWeights(alpha1, alpha2, a1, a2, i1, i2);
        updateCache();
        updateAlphas(a1, a2, i1, i2);
    ...
    public int takeStep(int i1, int i2){
        ...
        if(eta > 0){
            ...
            // System.out.println("Negative Derivative. Returning");
        ...
        updateALL(alpha1, alpha2, a1, a2, i1, i2);
        // Update b values
    ...
    public void Solver(){
        ...
        while(numchange > 0 || examineall ){
            ...
            if(examineall){
                ...
                myList = new ArrayList<>(nonBounded);
                for (int val : myList) {
    ...
    public void accCalc(double [][] data ,double [] test){
        int hit = 0;
        ...
        for (int row=0; row < data.length ; row++ ){
            ...
            for (int col=0; col < weights.length; col++){
                value += weights[col] *  data[row][col];
            }
            value = value - B;
        ...
        System.out.println("Accuracy -> " + (double) hit / data.length);
    }
